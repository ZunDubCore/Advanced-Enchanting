package zundubcore.advancedenchanting.util;
...
public final class MathUtils {
    ...
    public static double round(double value, int places) {
        ...
        return value >= 0 && places > 0 ? BigDecimal.valueOf(value).setScale(places,
                RoundingMode.HALF_UP).doubleValue() : value;
    }
    ...
    public static AxisAlignedBB getBoundsForPixels(int minX, int minY, int minZ, int maxX, int maxY,
        ...
        return new AxisAlignedBB(getPixelDistance(minX), getPixelDistance(minY),
                getPixelDistance(minZ), getPixelDistance(maxX), getPixelDistance(maxY),
                getPixelDistance(maxZ));
    }

    /**
     * Takes an integer value and fits it within a range. If the initial value is less than the
     * minimum it will be set to the minimum. If the initial value is greater than the maximum it
     * will be lowered to the maximum.
     *
     ...
     * @param factor The value to multiply the color by. Less than 1 will darken. Greater than 1
     *               will lighten.
